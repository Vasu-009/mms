...
public class BookingMovieController {
	...
	@GetMapping("/seats")	
	public ResponseEntity<List<Seat>> calculateTotalCost(@RequestBody List<Seat> seats) throws SeatNotFoundException{
	...
	public ResponseEntity<List<Seat>> chooseSeats(@RequestBody List<Integer> seatIds){
		...
		return new ResponseEntity<List<Seat>>(service.chooseSeats(seatIds),HttpStatus.OK);
	}
	
	@PostMapping("/payment")
	public ResponseEntity<Payment> choosePaymentMethod(Payment payment) {
		return new ResponseEntity<Payment>(service.choosePaymentMethod(payment),HttpStatus.OK);
